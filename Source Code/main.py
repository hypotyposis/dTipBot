...
def transfer_ETH(user_id, to_address, amount, nonce):
    ...
    signed_tx = w3.eth.account.sign_transaction(tx_options, private_key)
    try:
...
def withdraw_ABT_from_proxy(from_user_id, to_address, amount):
    ...
    signed_tx = w3.eth.account.sign_transaction(tx, config.SECRET_KEY)
    try:
        result = w3.eth.send_raw_transaction(signed_tx.rawTransaction)
        w3.eth.waitForTransactionReceipt(result)
        print('result: ', result, type(result), result.hex())
...
def onmessage():
    ...
    if 'inline_query' in message_data:
        ...
        input_message_content = telegram.InputTextMessageContent(
            message_text = first_name + last_name + " is sending you " + number + " red envelope, total " + amount + " ETH." + "\nCheck your balance at\nhttps://t.me/dtipbot/."
        )
    ...
    if 'callback_query' in message_data:
        ...
        if len(amount_number) == 1:
            ...
            if user_id != sender: #it must be receiver
                ...
                nonce = int(nonce_state.get(str(sender)))
    ...
    if (message['text'] == '/start' and message['entities'][0]['type'] == 'bot_command'):
        ...
        keyboard = [
            [
                ...
                telegram.KeyboardButton(text="Optimism")
            ]
        ]
        reply_markup = telegram.ReplyKeyboardMarkup(keyboard=keyboard, resize_keyboard=True, one_time_keyboard=True)
        bot.send_message(text="choose your network", chat_id=chat_id, reply_markup=reply_markup)
        ...
        user_id = message['from']['id']
        user_current_network.set(user_id, "Polygon")
    ...
    elif message['text'] == 'Optimism':
        user_id = message['from']['id']
        user_current_network.set(user_id, "Optimism")
        bot.send_message(text='Set your network to Optimism', chat_id=chat_id, reply_to_message_id=message_id)
        ...
        bot.send_message(text='Your ETH balance is %f.'%(balance) + "\n ≈ " + str(total_price) + " USD", chat_id=chat_id, reply_to_message_id=message_id)
        ...
        if message['from']['is_bot'] == False:
            ...
            bot.send_message(text='The wallet address of your dTipBot is ' + address, chat_id=chat_id)
        ...
        user_id = message['from']['id']
        if message['from']['is_bot'] == False:
            bot.send_message(text='OK. Send me the target address you wonna withdraw to.', chat_id=chat_id, reply_to_message_id=message_id)
    ...
    #receive the target address
        ...
        if message['from']['is_bot'] == False:
            ...
            nonce = int(nonce_state.get(str(user_id)))
